...
class Var:
    ...
    BAN_SUPPORT = getenv("BAN_SUPPORT", "https://t.me/ITs_shiro_pro")
    FSUB_LINK_EXPIRY = int(getenv("FSUB_LINK_EXPIRY", "120"))
    CHANNEL_ID = int(getenv("CHANNEL_ID", "0"))
    MHCHANNEL_URL = getenv("MHCHANNEL_URL", "https://t.me/djbhjsbdjash")
    ANIME = getenv("ANIME", "Is It Wr2131ong to Try to Pi123ck Up Girls in a Dungeon?")
    CUSTOM_BANNER = getenv("CUSTOM_BANNER", "https://ibb.co/CZ0djBy")

    PROTECT_CONTENT = True if getenv('PROTECT_CONTENT', "False") == "True" else False 
    BACKUP_CHANNEL = int(getenv("BACKUP_CHANNEL", "-1001753514086"))
    LOG_CHANNEL = int(getenv("LOG_CHANNEL", "-1003548938800"))
    MAIN_CHANNEL = int(getenv("MAIN_CHANNEL", "-1003679136727"))
    FILE_STORE = int(getenv("FILE_STORE", "-1001753514086"))
    ADMINS = list(map(int, getenv("ADMINS", "5556475282").split()))

    RSS_ITEMS = getenv("RSS_ITEMS", "").split()
    SEND_SCHEDULE = getenv("SEND_SCHEDULE", "True").lower() == "true"
    BRAND_UNAME = getenv("BRAND_UNAME", "@ANIME_X_FLEX")
    ...
    THUMB = getenv("THUMB")
    START_PIC = getenv("START_PIC","https://ibb.co/xttfrkw9")
    FORCE_PIC = getenv("FORCE_PIC", "https://ibb.co/ch3njSw4")
